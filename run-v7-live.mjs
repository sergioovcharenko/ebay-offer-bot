...
source = source.replace(requestOfferPattern, String.raw`async function requestOfferSend(userId,offerId,amount){const or=await pool.query('SELECT * FROM offers WHERE id=$1 AND user_id=$2',[offerId,userId]);if(!or.rowCount){const e=Error('Offer not found');e.status=404;throw e;}const offer=or.rows[0];const value=round(Number(amount));if(!(value>0))throw Error('Сума пропозиції має бути більше 0');if(DRY_RUN){const e=Error('Реальне надсилання вимкнено (DRY RUN). Пропозицію продавцю не відправлено.');e.status=409;throw e;}const result=await placeBestOffer(userId,offer.item_id,value);await pool.query("UPDATE offers SET amount=$1,status='OFFER_SENT',best_offer_id=$2 WHERE id=$3 AND user_id=$4",[value,result.bestOfferId||'',offerId,userId]);await notifyUser(userId,'eBay Offer Bot','Пропозицію реально відправлено продавцю: '+offer.title+'\n'+value+' '+offer.currency);return {ok:true,status:'OFFER_SENT',bestOfferId:result.bestOfferId||'',amount:value,message:'Пропозицію реально відправлено продавцю',dryRun:DRY_RUN};}
