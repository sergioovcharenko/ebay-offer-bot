...
async function addRule(){try{const mode=document.querySelector('#mode').value,v=+offerValue.value;await api('/api/rules',{method:'POST',body:JSON.stringify({name:name.value,productName:product.value,searchKeywords:keywords.value,sizeVariant:size.value,condition:condition.value,maxListingPrice:+maxListing.value,requireBestOffer:true,offerMode:mode,discountPercent:mode==='percentage'?v:undefined,fixedOfferAmount:mode==='fixed'?v:undefined,offerStepAmount:+step.value,maxOfferAmount:+maxOffer.value})});await load()}catch(e){alert(e.message)}}
