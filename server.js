...
function validateRule(x){
  for(const k of ['name','productName','searchKeywords']) if(!String(x[k]||'').trim()) throw new Error(k+' is required');
  ...
  else {fixedOfferAmount=Number(x.fixedOfferAmount);if(!(fixedOfferAmount>0))throw new Error('Фіксована пропозиція має бути більше 0');}
  return {id:x.id||randomUUID(),name:String(x.name).trim(),productName:String(x.productName).trim(),searchKeywords:String(x.searchKeywords).trim(),requiredKeywords:arr(x.requiredKeywords),excludedKeywords:arr(x.excludedKeywords),sizeVariant:String(x.sizeVariant||'').trim(),condition:String(x.condition||'ANY'),maxListingPrice,requireBestOffer:x.requireBestOffer!==false,offerMode,discountPercent,fixedOfferAmount,offerStepAmount,maxOfferAmount,enabled:x.enabled!==false,createdAt:x.createdAt||new Date().toISOString()};
}
...
function matches(r,l){
  const t=txt(l.title);
  for(const w of txt(r.productName).split(' ').filter(Boolean)) if(!t.includes(w)) return false;
...
async function searchEbay(rule){
  if(PROVIDER==='fake') return [{itemId:'fake-'+Date.now(),title:`${rule.productName} ${rule.searchKeywords} ${rule.sizeVariant}`.trim(),price:Math.min(rule.maxListingPrice,100),currency:'USD',condition:rule.condition==='ANY'?'USED':rule.condition,buyingOptions:['FIXED_PRICE','BEST_OFFER'],itemUrl:'https://www.ebay.com/',imageUrl:''}];
  const token=await ebayToken(); const q=encodeURIComponent(`${rule.productName} ${rule.searchKeywords}`);const filter=encodeURIComponent(`price:[0..${rule.maxListingPrice}],priceCurrency:USD`);
...
const CSS=`body{font-family:system-ui,-apple-system,sans-serif;margin:0;background:#f5f7fb;color:#172033}header{padding:18px;background:#111827;color:white;position:sticky;top:0}main{max-width:920px;margin:auto;padding:16px}.card{background:white;border-radius:16px;padding:16px;margin:12px 0;box-shadow:0 1px 5px #0001}.grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:10px}label{font-size:13px;color:#596273}input,select,button{width:100%;box-sizing:border-box;padding:12px;border:1px solid #d6dbe5;border-radius:10px;font-size:16px}button{background:#111827;color:white;border:0;font-weight:700}.secondary{background:#e9edf5;color:#172033}.danger{background:#8b1e1e}.row{display:flex;gap:8px}.row>*{flex:1}.muted{color:#6b7280;font-size:13px}.pill{display:inline-block;padding:4px 8px;border-radius:999px;background:#eef2ff;font-size:12px}.offer{border-top:1px solid #eee;padding-top:12px;margin-top:12px}@media(max-width:650px){.grid{grid-template-columns:1fr}header{position:static}}`;
const HTML=`<!doctype html><html lang="uk"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1,viewport-fit=cover"><meta name="theme-color" content="#111827"><link rel="manifest" href="/manifest.webmanifest"><title>eBay Offer Bot</title><style>${CSS}</style></head><body><header><b>eBay Offer Bot</b><div class="muted" style="color:#cbd5e1">Пошук + Best Offer</div></header><main><div class="card"><h3>Нове правило</h3><div class="grid"><input id="name" placeholder="Назва правила"><input id="product" placeholder="Товар / модель"><input id="keywords" placeholder="Ключові слова"><input id="size" placeholder="Розмір, напр. 34x32"><select id="condition"><option value="ANY">Будь-який стан</option><option value="NEW">Новий</option><option value="NEW_WITH_TAGS">Новий з бірками</option><option value="NEW_WITHOUT_TAGS">Новий без бірок</option><option value="OPEN_BOX">Open box</option><option value="USED">Вживаний</option><option value="REFURBISHED">Refurbished</option></select><input id="maxListing" type="number" placeholder="Макс. ціна товару"><select id="mode"><option value="fixed">Фіксована пропозиція</option><option value="percentage">% від ціни</option></select><input id="offerValue" type="number" placeholder="Сума або %"><input id="step" type="number" placeholder="Крок після відхилення"><input id="maxOffer" type="number" placeholder="Максимум пропозиції"></div><p><button onclick="addRule()">Додати правило</button></p></div><div class="card"><h3>Сповіщення</h3><div id="notify"></div></div><div class="card"><h3>Правила</h3><div id="rules"></div></div><div class="card"><h3>Пропозиції</h3><div id="offers"></div></div></main><script>
...
async function addRule(){try{const byId=id=>document.getElementById(id);const mode=byId('mode').value,v=+byId('offerValue').value;await api('/api/rules',{method:'POST',body:JSON.stringify({name:byId('name').value,productName:byId('product').value,searchKeywords:byId('keywords').value,sizeVariant:byId('size').value,condition:byId('condition').value,maxListingPrice:+byId('maxListing').value,requireBestOffer:true,offerMode:mode,discountPercent:mode==='percentage'?v:undefined,fixedOfferAmount:mode==='fixed'?v:undefined,offerStepAmount:+byId('step').value,maxOfferAmount:+byId('maxOffer').value})});await load()}catch(e){alert(e.message)}}
async function run(id){await api('/api/rules/'+id+'/run',{method:'POST'});await load()}
async function reject(id){await api('/api/offers/'+id+'/rejected',{method:'POST'});await load()}
async function load(){const [rules,offers,s]=await Promise.all([api('/api/rules'),api('/api/offers'),api('/api/settings')]);document.querySelector('#notify').innerHTML='<p class="muted">Встанови ntfy на телефоні та підпишись на канал:</p><b>'+s.notifications.notifyTopic+'</b><p><a target="_blank" href="'+s.notifications.notifyServer+'/'+s.notifications.notifyTopic+'">Відкрити канал</a></p>';document.querySelector('#rules').innerHTML=rules.length?rules.map(r=>'<div class="offer"><b>'+r.name+'</b><br><span class="muted">'+r.productName+' · '+r.searchKeywords+' · '+(r.sizeVariant||'будь-який розмір')+' · max '+r.maxListingPrice+' USD</span><p><button onclick="run(\\''+r.id+'\\')">Перевірити зараз</button></p></div>').join(''):'<p class="muted">Правил ще немає.</p>';document.querySelector('#offers').innerHTML=offers.length?offers.map(o=>'<div class="offer"><b>'+o.title+'</b><br><span class="pill">'+o.status+'</span><p>Ціна: '+o.listingPrice+' '+o.currency+'<br><b>Пропозиція: '+o.amount+' '+o.currency+'</b><br>Спроба: '+o.attempt+'</p><div class="row"><a href="'+o.itemUrl+'" target="_blank"><button class="secondary">eBay</button></a><button class="danger" onclick="reject(\\''+o.id+'\\')">Відхилено</button></div></div>').join(''):'<p class="muted">Пропозицій поки немає.</p>'}
...
const sw=`const C='ebay-offer-v1';self.addEventListener('install',e=>e.waitUntil(caches.open(C).then(c=>c.addAll(['/']))));self.addEventListener('fetch',e=>{if(e.request.method==='GET')e.respondWith(fetch(e.request).catch(()=>caches.match(e.request)))})`;
